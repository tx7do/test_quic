...
namespace quic::detail
{
	...
	namespace sending_stream_state
	{
		...
		void on_write(variant& state, lsquic_stream* handle)
		{
			...
			{
				assert(std::holds_alternative<body>(state));
		...
		int cancel(variant& state, error_code ec)
		{
			...
			{
				if (auto op = std::get_if<header>(&state)->op; op)
				{
					op->defer(ec);
				}
				state = shutdown{};
			...
			{
				if (auto op = std::get_if<body>(&state)->op; op)
				{
					op->defer(ec, 0);
				}
				state = shutdown{};
	...
	namespace receiving_stream_state
	{
		...
		void on_read_header(variant& state, lsquic_stream* handle)
		{
			...
			{
				auto headers = std::unique_ptr<recv_header_set>{ reinterpret_cast<recv_header_set*>(hset) };
	...
	namespace stream_state
	{
		...
		transition on_error(variant& state, error_code ec)
		{
			...
			{
				if (auto op = std::get_if<connecting>(&state)->op; op)
				{
					op->defer(ec);
				}
				state = closed{};
			...
			if (std::holds_alternative<closing>(state))
			{
				if (auto op = std::get_if<closing>(&state)->op; op)
				{
					op->defer(ec);
				}
				state = closed{};
