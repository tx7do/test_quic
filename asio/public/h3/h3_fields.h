...
namespace h3
{
	...
	private:
		...
		static ptr create(std::string_view name, std::string_view value, bool never_index)
		{
	...
	public:
		...
		{
			set.erase(set.iterator_to(*begin), std::next(set.iterator_to(*std::prev(end))));
